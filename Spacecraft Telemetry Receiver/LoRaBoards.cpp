 ...
 * @author    Tanay C.
 * @license   MIT
 * @copyright Copyright (c) 2025  Tanay C.
 * @date      2025-04-22
...
bool beginDisplay()
{
    ...
    if (Wire.endTransmission() == 0) {
        ...
        u8g2->begin();
        u8g2->setDisplayRotation(U8G2_R2); // Use setDisplayRotation instead
        u8g2->clearBuffer();

        u8g2->setFont(u8g2_font_helvB12_tf); // Using Helvetica Bold (helvB12_tf)
        int lilygo_width = u8g2->getStrWidth("LoRa");
        int lilygo_x = (128 - lilygo_width) / 2; // Calculate x-coordinate to center "LilyGo"
        u8g2->drawStr(lilygo_x, 16, "LoRa"); // Centered "LilyGo" - Adjusted Y position

        u8g2->setFont(u8g2_font_helvB12_tf); // Using Helvetica Bold for "LoRa" as well
        int lora_width = u8g2->getStrWidth("Telemetry");
        int lora_x = (128 - lora_width) / 2;     // Calculate x-coordinate to center "LoRa"
        u8g2->drawStr(lora_x, 36, "Telemetry"); // Centered "LoRa" - Adjusted Y position
